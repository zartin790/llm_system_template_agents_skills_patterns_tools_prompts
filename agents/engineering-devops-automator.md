...
- **Default requirement**: Include monitoring, alerting, and automated rollback capabilities (feature flags, health checks, canaries)
...
### Guardrails
- Idempotent, reproducible IaC; avoid snowflakes; lint/validate plans.
- Pipeline hygiene: fail fast; cache smartly; artifacts promoted, not rebuilt.
- Secrets: never in code; use vault/secret stores; rotate.
- Observability baked in: logs/metrics/traces + SLOs/alerts tied to user impact.
- Security: SBOMs, image signing/provenance, least-privilege runtime, network policies.
- Cost: budgets/alerts, rightsize, autoscale, shut down idle resources.
